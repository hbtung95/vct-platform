...
function getRoundLabel(roundIndex: number, totalRounds: number): string {
  ...
  if (fromEnd >= 0 && fromEnd < ROUND_LABELS.length) {
    return ROUND_LABELS[ROUND_LABELS.length - 1 - fromEnd] ?? `Vòng ${roundIndex + 1}`
  }
                  ...
                  >
                    <VctCard style={styles.matchCard}>
                      <VctBadge
                        ...
                        variant={match.status === 'completed' ? 'success' : match.status === 'in_progress' ? 'error' : 'neutral'}
                      />
